...
export const updateOrderTotalPrice = (orderId, callback) => {
  const sql = `
    SELECT 
      o.unit_price,
  ...
  db.query(sql, [orderId], (err, result) => {
    if (err) {
      console.error("Error calculating order total price:", err);
      if (callback) callback(err);
      return;
    }

    if (result.length === 0) {
      if (callback) callback(new Error("Order not found"));
      return;
    }
    ...
    db.query(updateSql, [new_total_price, orderId], (updateErr) => {
      if (updateErr) {
        console.error("Error updating order total price:", updateErr);
        if (callback) callback(updateErr);
        return;
      }
      
      if (callback) callback(null, new_total_price);
    });
  });
};
...
router.get("/", verifyToken, (req, res) => {
  const sql = `    SELECT o.*, c.name as client_name, p.name as product_name,
...
router.post("/", verifyToken, validateOrder, (req, res) => {
  ...
  db.query(
    getNamesSql,
    [fk_order_client, fk_order_product],
    (err, namesData) => {
      if (err) {
...
router.get("/production-requirements", verifyToken, (req, res) => {
  const sql = `
    SELECT 
      p.id as product_id,
