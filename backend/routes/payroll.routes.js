...
router.get("/employee/:employeeId", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error fetching employee payroll", details: error.message });
  }
});
...
router.get("/summary/range", verifyToken, async (req, res) => {
  try {
    const { start_date, end_date } = req.query;
    const [data] = await db.query(`
      SELECT 
        et.name as department,
    ...
    res.status(500).json({ error: "Error fetching payroll summary", details: error.message });
  }
});

// Generate salary slip
router.get("/salary-slip/:payrollId", verifyToken, async (req, res) => {
  try {
    const [data] = await db.query(`
      SELECT 
        p.*,
    ...
    res.status(500).json({ error: "Error generating salary slip", details: error.message });
  }
});
...
router.get("/calculate", verifyToken, async (req, res) => {
  try {
    ...
    if (!month || !/^\d{4}-\d{2}$/.test(month)) {
      return res.status(400).json({ error: "Invalid or missing month (YYYY-MM)" });
    }
    ...
    const results = await Promise.all(
      employees.map(async (emp) => {
        try {
          ...
          const salaryForDays = (baseSalary * Math.min(presentDays, 25)) / 25;
          ...
          return { ...emp, error: "Error fetching attendance" };
        }
      })
    );
    ...
    res.status(500).json({ error: "Error calculating payroll", details: error.message });
  }
});
